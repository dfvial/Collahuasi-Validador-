...
namespace LiOriginal
{
    class UserStep1Definition : IStepDefinition
    {
        ...
        static readonly Guid MY_ID = new Guid("{8e734155-72ff-441b-beb4-de1dedb465ba}");
        ...
        public ExitType Execute(IStepExecutionContext context)
        {
            ...
            */
            ...
            for (int j = 0; j < Npalas; j++)
            {
                if (Candidata[j] > 0)
                {                    
                    tij_op[j] = 1 / (vectores.PlYv[Convert.ToInt32(vectores.DesCam[Convert.ToInt32(idcamion) - 1, 2]-1), j]);
